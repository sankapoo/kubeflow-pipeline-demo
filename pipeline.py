...
@dsl.component(base_image=DATA_SCIENCE_IMAGE)
def predict_sentiment(texts: list, labels: list, input_text: str) -> str:
    from sklearn.feature_extraction.text import TfidfVectorizer
    from sklearn.naive_bayes import MultinomialNB
    from sklearn.pipeline import make_pipeline
...
def pipeline():
    ...
    verify_accuracy(accuracy=accuracy, threshold=0.80)

    # NEW: Predict sentiment of a live input
    predict_task = predict_sentiment(
        texts=texts,
        labels=labels,
        input_text="This is wonderful, I am happy to see this report."
    )
    predict_task.set_caching_options(False)
